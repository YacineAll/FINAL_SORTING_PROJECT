...
int Rech_Circuit(Graphe *H, int i, int j, int i_cour, int j_cour){
  int k, l;
  Arc *courant = NULL;
  H->Tsom[i_cour][j_cour]->visit = 0;
  courant = H->Tsom[i_cour][j_cour]->Lsucc;
  while (courant != NULL) {

    k = courant->succ->i;
    l = courant->succ->j;

    //si le sommet courant est le sommet initial ou il n'a pas été visité mais il appartient au circuit
    if ((H->Tsom[k][l]->visit == 0 && k == i && l == j) || (H->Tsom[k][l]->visit == -1 && Rech_Circuit(H, i, j, k, l))) {
      printf("<-(%d,%d)", i_cour, j_cour);
      return 1;
    }
...
Cell_circuit* Rech_circuit_v2 ( Graphe *G , int i ,int j ){
  ...
  for ( i = 0 ; i  < G->m ; i++)
    for ( j = 0 ; j < G->n ; j++){
      if ( G->Tsom[i][j]->visit == -1 && G->Tsom[i][j]->visit != -2 ){
          temp = Rech_circuit_v2(G , i , j);
          if (temp)
            LCInsererEnFin(lcir , temp->L , temp->jmin ,temp->jmax); 
      }
    }
...
void Ajout_circuit_dans_solution(Cell_circuit *Circ, Solution* S, Cell_char** Tref, int* Jdroite){
  ...
  while ( temp ){
    l = temp->j;
    j = cour->j;
    if ( j > *Jdroite) *Jdroite = j;
    cell = Ajout_action_apres_c(S , cell , j , 'S' , Tref);
...
void algorithme_circuit_CasLigne1x1(Grille *G , Solution *S){
  ...
  while ( Circ){
    if ( !Tref[Circ->jmin] ){  
      ...
      cell = Ajout_action_apres_c(S , cell , Circ->jmin , 'S' , Tref);
      if ( Circ->jmax > Jdroite) Jdroite = Circ->jmax; 
    }
